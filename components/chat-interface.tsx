...
export function ChatInterface({ agents }: { agents: Agent[] }) {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-120px)] md:h-[calc(100vh-100px)] max-w-4xl mx-auto gap-4">

            {/* Top Bar: Online Agents */}
            <div className="bg-slate-900/40 border border-white/5 rounded-xl p-3 flex items-center gap-4 overflow-x-auto">
                <div className="text-xs font-mono text-slate-500 whitespace-nowrap">ONLINE AGENTS:</div>
                <div className="flex items-center gap-2">
                    {agents.map(agent => (
                        <div key={agent.id}
                            onClick={() => setInput(prev => `${prev} @${agent.name} `)} // Tagging helper
                            className={`flex items-center gap-1.5 px-2 py-1 rounded-full border text-[10px] cursor-pointer transition-colors ${agent.status === 'RUNNING'
                                    ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20'
                                    : 'bg-slate-800 border-white/5 text-slate-500 hover:text-slate-300'
                                }`}>
                            <div className={`w-1.5 h-1.5 rounded-full ${agent.status === 'RUNNING' ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'}`} />
            ...
            <div className="flex-1 flex flex-col relative min-h-0">
                ...
                <form onSubmit={sendMessage} className="mt-4 flex gap-2">
                    <input
                        ...
                        placeholder="Broadcast message to swarm... (Type @Agent to target)"
                        className="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 transition-all font-mono"
                    />
                    <button
                        type="submit"
